...
describe("TimeSeriesService", () => {
  ...
  describe("create", () => {
    ...
    it("stores the series and its points in one transaction, skipping duplicates", async () => {
      ...
      prisma.timeSeries.findUniqueOrThrow.mockResolvedValue(
        seriesWithSummary as never,
      );
      ...
      expect(prisma.dataPoint.createMany).toHaveBeenCalledWith({
        data: [
          ...
          {
            seriesId: "series-1",
            timestamp: new Date("2026-09-20T10:01:00.000Z"),
            value: 3,
          },
        ],
        skipDuplicates: true,
    ...
    it("returns 404 when the sensor does not belong to the user", async () => {
      ...
      await expect(
        service.create(USER_ID, "other", dto),
      ).rejects.toBeInstanceOf(NotFoundException);
      expect(prisma.sensor.findFirst).toHaveBeenCalledWith({
        where: {
          id: "other",
          monitoringPoint: { machine: { userId: USER_ID } },
        },
      });
  ...
  it("appends points and reports how many were inserted", async () => {
    ...
    const result = await service.appendPoints(USER_ID, "series-1", [
      { timestamp: "2026-09-20T10:02:00.000Z", value: 4 },
    ]);
  ...
  it("counts only the series of the user, optionally filtered by sensor", async () => {
    ...
    expect(prisma.timeSeries.count).toHaveBeenLastCalledWith({
      where: { ...OWNED, sensorId: "sensor-1" },
    });
  });
  ...
  it("filters the points by time range when from/to are given", async () => {
    ...
    prisma.dataPoint.findMany.mockResolvedValue([
      { timestamp: new Date("2026-09-20T10:01:00.000Z"), value: 3 },
    ] as never);
    ...
    expect(prisma.dataPoint.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          seriesId: "series-1",
          timestamp: {
            gte: new Date("2026-09-20T10:01:00.000Z"),
            lte: new Date("2026-09-20T10:03:00.000Z"),
          },
        },
  ...
  describe("metrics", () => {
    it("returns the database aggregates with ISO dates", async () => {
      prisma.timeSeries.findFirst.mockResolvedValue({
        id: "series-1",
      } as never);
    ...
    it("restricts the aggregates to the requested period", async () => {
      prisma.timeSeries.findFirst.mockResolvedValue({
        id: "series-1",
      } as never);
      prisma.$queryRaw.mockResolvedValue([
      ...
      await service.metrics(USER_ID, "series-1", {
        from: "2026-09-20T10:00:00.000Z",
        to: "2026-09-20T10:05:00.000Z",
      });

      const [, where] = prisma.$queryRaw.mock.calls[0] as [
        unknown,
        { sql: string; values: unknown[] },
      ];
      expect(where.sql).toContain('"timestamp" >=');
      expect(where.sql).toContain('"timestamp" <=');
  ...
  it("does not delete a series that belongs to another user", async () => {
    ...
    await expect(service.remove(USER_ID, "other")).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(prisma.timeSeries.findFirst).toHaveBeenCalledWith({
      where: { id: "other", ...OWNED },
    });
